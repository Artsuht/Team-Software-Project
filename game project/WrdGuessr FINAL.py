...
first = 0  #Checks that it's the first turn for instructions
progress = False
...
while running:
                ...
                if next_button.check_click(mouse_pos) or success_loss_menu_button.check_click(mouse_pos):
                    ...
                    if next_button.check_click(mouse_pos):  #progression check
                        if GAME_STATE == "PROGRESSION":
                            ...
                            if (current_difficulty == "EASY" and words_attempted == 10) or (current_difficulty != "EASY" and words_attempted == 20) or check == True:
    ...
    if GAME_STATE == "MENU":
        ...
        if current_difficulty == "EASY":
            correct_amount_text = font.render(f"{correct_words}/10", True, (0, 0, 0))
        else:
            correct_amount_text = font.render(f"{correct_words}/20", True, (0, 0, 0))
        screen.blit(correct_amount_text, (WINDOW_WIDTH - 150, 60))

        turn_text = font.render(f"Turn: {words_attempted + 1}", True, (0, 0, 0))
        screen.blit(turn_text, (WINDOW_WIDTH // 2 - 125, WINDOW_HEIGHT - 100))

        screen.blit(hangman_images[current_image_index], (WINDOW_WIDTH - 750, WINDOW_HEIGHT // 2 - 412))

        if first == 0:
            start_text = font3.render("You need to guess the letters in the word - You need 7/10 to progress to medium", True, (0, 0, 0))
            screen.blit(start_text, (WINDOW_WIDTH // 2 - 375, 10))
        ...
        if GAME_STATE == "SUCCESS" or GAME_STATE == "LOSS":
            ...
            if current_difficulty == "EXTREME" and correct_in_row == 20 and progress == False:
                print("YOU WIN THE GAME!")
                GAME_STATE = "WIN_ALL"
                progress = True
            elif current_difficulty == "HARD" and words_attempted == 20 and correct_words >= 14 and progress == False:
                print("PROGRESSING FROM HARD TO EXTREME!")
                current_difficulty = "EXTREME"
                progress = True
            elif current_difficulty == "MEDIUM" and words_attempted == 20 and correct_words >= 14 and progress == False:
                print("PROGRESSING FROM MEDIUM TO HARD!")
                current_difficulty = "HARD"
                progress = True
            elif current_difficulty == "EASY" and words_attempted == 10 and correct_words >= 7 and progress == False:
            ...
            if current_difficulty == "EASY":
                progression_limit_text = font.render(f"To progress you need 7 / 10", True, (255, 255, 255))
                ...
                progression_limit_text = font.render(f"To progress you need 14 / 20", True, (255, 255, 255))
                correct_words_text = font.render(f"You got {score} / 20", True, (255, 255, 255))
            else:
                progression_limit_text = font.render(f"To progress you need 20 / 20", True, (255, 255, 255))
                correct_words_text = font.render(f"You got {score} / 20", True, (255, 255, 255))
